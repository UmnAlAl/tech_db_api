...
BEGIN
    ...
    */
    ...
    ELSE /* если есть родитель */
        ...
        SET cnt = cnt + 1; /* величить счетчик */
...
BEGIN
	IF (ord = 'desc') THEN
		SELECT CAST(SUBSTRING( post.matPath, 1, LOCATE('/', post.matPath) - 1) AS UNSIGNED ) AS firstIndex, post.* FROM post
        WHERE post.idThread = threadId AND post.date >= sinceDate
...
-- Dump completed on 2016-03-27 20:09:23
